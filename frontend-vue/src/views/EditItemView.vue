      ...
      <form @submit.prevent="handleSave" class="escambo-card p-3">
        ...
        <div class="mb-3">
          ...
          <select v-model="form.category" class="form-select" required>
            ...
            <option value="música">Instrumentos</option>
            <option value="jogos">Games</option>
...
onMounted(async () => {
  try {
    ...
    // Normaliza categoria para minúsculo (backend espera minúsculo)
    form.category = (loaded.category || '').toLowerCase();
    // Normaliza condição para minúsculo
    form.condition = (loaded.condition || '').toLowerCase();
