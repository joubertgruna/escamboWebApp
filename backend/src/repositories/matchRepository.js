...
class MatchRepository {
  ...
  async findByIdWithDetails(id) {
    ...
    // Get photos for both items
    const [photos1, photos2] = await Promise.all([
      match.item_1_id ? db('photos').where({ item_id: match.item_1_id }).select('id', 'url') : [],
      match.item_2_id ? db('photos').where({ item_id: match.item_2_id }).select('id', 'url') : [],
    ]);
    ...
    return {
      ...
      item_2: item2 ? { ...item2, photos: photos2 } : null,
    };
  }
