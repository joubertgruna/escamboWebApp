...
class ItemRepository {
  ...
  async findForFeed(userId, { limit, offset }) {
    const items = await db('items')
      .select('items.*')
      .where('status', 'active')
      .whereNot('user_id', userId)  // Exclude own items from feed
      .orderBy('created_at', 'desc')
      .limit(limit)
      .offset(offset);

    // Attach primary photo and user data
    const itemIds = items.map((i) => i.id);
    ...
    const [{ total }] = await db('items')
      .where('status', 'active')
      .whereNot('user_id', userId)  // Exclude own items from count
      .count('* as total');
